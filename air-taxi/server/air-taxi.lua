...
chatTextColor1    = Color(192, 192, 192) -- color for normal messages
chatTextColor2    = Color(255, 0, 0)   -- color for warnings
chatPrefix        = "[Airliner Co.] "            -- text that shows in fornt of every message
...
function Driver:AddPassenger(player)
	...
	Chat:Send(player, chatPrefix .. "You are passenger of Pilot " .. self.player:GetName() .. " now, this flight costs you " .. math.floor(price * passengerTax) .. "$ per km", chatTextColor1) 
end
...
function Driver:RemovePassenger(index, death)
		...
		if death then
			...
			Chat:Send(player, chatPrefix .. "Pilot " .. self.player:GetName() .. " pays back your taxes (" .. p.payed .. "$) and a penalty (" .. penalty .. "$)", chatTextColor2)
		else
...
function AirTaxi:AddDriver(args)
	local driver = Driver(args.player)
	table.insert(self.drivers, driver)
	Chat:Send(args.player, chatPrefix .. "You are Pilot " .. tostring(#self.drivers) .. " now", chatTextColor1) 
...
function AirTaxi:RemoveDriver(index)
	Chat:Send(self.drivers[index].player, chatPrefix .. "You left your plane, you earned " .. self.drivers[index].earned .. "$ with your last flight", chatTextColor1) 
	table.remove(self.drivers, index)
end

function AirTaxi:Update(args)
...
function AirTaxi:EnterVehicle(args)
...
function AirTaxi:ExitVehicle(args)
...
function AirTaxi:PlayerDeath(args)
	local search = self:FindPlayer(args.player)
	if search then
		if search.passenger then
...
function AirTaxi:FindPlayer(player)
	local d, p
	for i=1, #self.drivers, 1 do
		d = self.drivers[i]
...
function AirTaxi:CheckVehicle(id)

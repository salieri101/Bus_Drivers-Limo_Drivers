...
chatTextColor1    = Color(173, 216, 230) -- color for normal messages
chatTextColor2    = Color(255, 0, 0)   -- color for warnings
chatPrefix        = "[Bus Co.] "            -- text that shows in fornt of every message
...
function Driver:AddPassenger(player)
	...
	Chat:Send(player, chatPrefix .. "You are passenger of Bus driver " .. self.player:GetName() .. " now, this cab costs you " .. math.floor(price * passengerTax) .. "$ per km", chatTextColor1) 
end
...
function Driver:RemovePassenger(index, death)
		...
		if death then
			...
			Chat:Send(player, chatPrefix .. "Bus driver " .. self.player:GetName() .. " pays back your taxes (" .. p.payed .. "$) and a penalty (" .. penalty .. "$)", chatTextColor2)
		else
...
function BusDrivers:AddDriver(args)
	local driver = Driver(args.player)
	table.insert(self.drivers, driver)
	Chat:Send(args.player, chatPrefix .. "You are Bus driver " .. tostring(#self.drivers) .. " now", chatTextColor1) 
...
function BusDrivers:RemoveDriver(index)
	Chat:Send(self.drivers[index].player, chatPrefix .. "You left your bus, you earned " .. self.drivers[index].earned .. "$ with your last ride", chatTextColor1) 
	table.remove(self.drivers, index)
end

function BusDrivers:Update(args)
...
function BusDrivers:EnterVehicle(args)
...
function BusDrivers:ExitVehicle(args)
...
function BusDrivers:PlayerDeath(args)
	local search = self:FindPlayer(args.player)
	if search then
		if search.passenger then
...
function BusDrivers:FindPlayer(player)
	local d, p
	for i=1, #self.drivers, 1 do
		d = self.drivers[i]
...
function BusDrivers:CheckVehicle(id)

...
chatPrefix        = "[Taxi Co.] "            -- text that shows in fornt of every message
...
function Driver:AddPassenger(player)
	...
	Chat:Send(player, chatPrefix .. "You are passenger of Taxi driver " .. self.player:GetName() .. " now, this cab costs you " .. math.floor(price * passengerTax) .. "$ per km", chatTextColor1) 
end
...
function Driver:RemovePassenger(index, death)
		...
		if death then
			...
			Chat:Send(player, chatPrefix .. "Taxi driver " .. self.player:GetName() .. " pays back your taxes (" .. p.payed .. "$) and a penalty (" .. penalty .. "$)", chatTextColor2)
		else
...
function TaxiDrivers:AddDriver(args)
	...
	Chat:Send(args.player, chatPrefix .. "You are Taxi driver " .. tostring(#self.drivers) .. " now", chatTextColor1) 
...
function TaxiDrivers:RemoveDriver(index)
	Chat:Send(self.drivers[index].player, chatPrefix .. "You left your taxi, you earned " .. self.drivers[index].earned .. "$ with your last ride", chatTextColor1) 
